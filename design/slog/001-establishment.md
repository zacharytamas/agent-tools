...
- Represent domain and infrastructure failures as typed errors that can be translated into the stable machine error envelope at the command boundary. V1 deliberately models all failures as a single `SlogError` (a plain `Error` subclass carrying a `_tag`, a `code` discriminator, and structured `details`) rather than per-variant `Schema.TaggedError` classes. Rationale: the CLI already hand-serializes a stable public `{ error: { code, message, details } }` envelope (so Schema serializability buys little), discrimination is on the `code` field rather than tag-based `catchTag` dispatch, and a single error type keeps construction uniform across the storage/lock/command layers. The cost is that errors are raised with `Effect.fail(new SlogError(...))` rather than yielded directly. Revisit (consider `Schema.TaggedError` + `catchTag`/`catchTags`) only if multiple error types emerge that genuinely need tag-based recovery dispatch.
...
Command parsing should stay thin. Domain validation should remain pure. Storage should stay behind an entry-shaped repository/service seam rather than exposing partition/file operations to command handlers. Command/domain code owns mutation policy; the repository owns persistence mechanics and storage invariants such as locking, duplicate detection, and safe rewrites. Modules may be split later when real size or cohesion pressure justifies it, but the v1 implementation should avoid a premature folder hierarchy.
...
3. **Mutation and triage**
   ...
   - Implement human `slog edit` inline flags for `text` and `occurred_at`, including `--clear-occurred-at` as the human equivalent of machine `occurred_at: null`. Human edit does not change triage state; `needs_triage` changes are handled through `slog triage resolve/reopen`. No-op human edit/triage commands succeed but report that no changes were made. Human mutation output is single-line `verb + full ULID + two spaces + truncated text`, where the snippet uses post-update text truncated to 60 characters with a trailing `…` only when cut and internal newlines collapsed to spaces; verbs are `Updated`, `Resolved`, `Reopened`, and `No changes`. `Entry not found: <id>` and `Storage corrupt: multiple records found for <id>` are non-zero-exit human errors with no snippet.
   - Implement `slog triage`, `slog triage --all`, `slog triage resolve <full-ulid>`, and `slog triage reopen <full-ulid>`. `slog triage` defaults to today-only; `--all` is the explicit full unresolved backlog sweep. Phase 3 does not need backlog counts in the default triage output. Triage list output reuses the `slog list` grouped-by-day single-line shape with full ULIDs, but omits the redundant `TRIAGE` marker because every row needs triage; `--all` groups by day with date headers, newest day first. `slog triage --all` discovers entries by recursively scanning valid daily JSONL partition paths under `~/.slog/entries/`; Phase 3 does not introduce an index/cache, but storage discovery stays behind the repository service so a later faster layer can replace JSONL scanning without changing command handlers. Phase 3 edit/update/triage commands continue requiring full ULIDs only; triage output should print full IDs for copy/paste.
...
- Repository mutation applies a validated patch under the partition lock to the latest stored entry, rather than replacing a caller-provided whole-entry snapshot. This avoids lost updates for disjoint concurrent field changes without introducing revisions or optimistic concurrency. If two concurrent updates change the same field, the last completed writer wins for that field.
...
- A lock older than a hardcoded max age (~60s, far beyond any legitimate millisecond-scale critical section) is treated as stale: the lock service reclaims it and proceeds. V1 uses age only (from `acquired_at`/mtime), not PID liveness checks, and accepts the negligible theoretical risk of reclaiming a pathologically slow live writer's lock.
- `storage_corrupt` / `duplicate_entry_id` is always a hard, non-zero-exit error, never a warning and never a silent degrade-and-continue. Machine surfaces emit the standard error envelope on stderr with the partition path in `details.path`; human surfaces emit a single-line stderr error. No entry or success envelope is written on stdout.
